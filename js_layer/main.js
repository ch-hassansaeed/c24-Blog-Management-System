...
//get base path of project
	...
	//Add post for main home page model popup box
	$("#post_form").on("submit",function(){
		if ($("#post_title").val() == "") {
			$("#post_title").addClass("border-danger");
			$("#post_error").html("<span class='text-danger'>Please Enter post Name</span>");
		}else{
			$.ajax({
				...
				success : function(data){
					if (data == "POST_ADDED") {
						$("#post_title").removeClass("border-danger");
						$("#post_error").html("<span class='text-success'>New post Added Successfully..!</span>");
						$("#post_title").val("");
						//fetch_post();
					}else{
						alert(data);
